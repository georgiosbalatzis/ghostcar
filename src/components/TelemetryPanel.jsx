...
const TelemetryPanel = memo(function TelemetryPanel({ mob, tp, prog, allDrivers, numDrivers, di1, di2, co1, co2, li1, li2, s1, s2, laps1, st1, sl1 }) {
  ...
  const speedTraps = useMemo(() => {
    ...
    return [0.1, 0.25, 0.5, 0.75, 0.9].map((t, i) => {
      ...
      return { v1, v2, faster: v1 > v2 ? 1 : v2 > v1 ? 2 : 0, label: ["T1 10%", "T2 25%", "T3 50%", "T4 75%", "T5 90%"][i] };
    });
  }, [s1, s2]);
  ...
  const tireDegData = useMemo(() => {
    ...
    const valid = laps1.filter((l) => l.lap_duration > 10 && l.lap_duration < 200).sort((a, b) => a.lap_number - b.lap_number);
        ...
        {allDrivers.map((x, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, minWidth: numDrivers > 2 ? 120 : "auto", background: F1.cardBg, borderRadius: 6, padding: numDrivers > 2 ? "6px 6px 4px" : "8px 8px 6px", borderTop: `3px solid ${x.co}`, position: "relative", textAlign: "center" }}>
              ...
              {ct.drs >= 10 && <div style={{ position: "absolute", bottom: 3, right: 4, fontSize: 7, fontWeight: 700, color: F1.green, fontFamily: F1.mono, background: `${F1.green}15`, padding: "1px 3px", borderRadius: 2, animation: "pulse 1s infinite" }}>DRS</div>}
            </div>
          );
        })}
      </div>
          ...
          <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
            {speedTraps.map((trap, i) => (
              <div key={i} style={{ flex: 1, minWidth: 48, background: F1.cardBg, borderRadius: 3, padding: "3px 4px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 800, color: trap.faster === 1 ? co1 : trap.faster === 2 ? co2 : F1.text, fontFamily: F1.mono }}>{Math.round(trap.v1)}</div>
                <div style={{ fontSize: 11, fontWeight: 800, color: trap.faster === 2 ? co2 : trap.faster === 1 ? co1 : F1.text, fontFamily: F1.mono, opacity: 0.7 }}>{Math.round(trap.v2)}</div>
              </div>
            ))}
          </div>
        </div>
