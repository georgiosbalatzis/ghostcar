...
export default function DashModal({ mob, year, di1, di2, co1, co2, dashData, onClose, inline }) {
  ...
  const wrapStyle = inline
    ...
    : getModalSurfaceStyle(F1, { mob, width: 600, maxHeight: "85vh" });
  return (
    <div style={wrapStyle}>
      <div style={getModalHeaderStyle(F1)}>
        <div>
          ...
          <div style={{ fontSize: 10, color: F1.textMuted }}>
            {di1?.name_acronym || "Ο1"} εναντίον {di2?.name_acronym || "Ο2"} — Εικόνα κατατακτήριων
          </div>
        </div>
        <button aria-label="Κλείσιμο ανάλυσης σεζόν" onClick={onClose} style={getModalCloseButtonStyle(F1)}>
          ✕
        </button>
      </div>
        ...
        {!dashData ? (
          ...
          <>
            <div style={{ marginBottom: 16 }}>
              ...
              >
                <span style={{ color: co1, fontWeight: 900 }}>
                  {di1?.name_acronym} {summary?.w1 || 0}
                </span>
                <span style={{ color: F1.textMuted, fontSize: 10 }}>ΝΙΚΕΣ ΣΕ ΚΑΤΑΤΑΚΤΗΡΙΕΣ</span>
                <span style={{ color: co2, fontWeight: 900 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: F1.mono }}>
              ...
              <tbody>
                {dashData.map((r, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: `1px solid ${F1.borderLight}` }}>
                      ...
                      <td
                        style={{
                          padding: "6px 4px",
                          textAlign: "center",
                          fontWeight: winner === 2 ? 800 : 400,
                          color: winner === 2 ? co2 : F1.text,
                        }}
                      >
                        {fmt(r.t2)}
                      </td>
                      <td style={{ padding: "6px 4px", textAlign: "center", color: winnerColor, fontWeight: 700 }}>
                        {r.d > 0 ? "+" : ""}
                        {r.d.toFixed(3)}
                      </td>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              >
                ...
                <text x="5" y="10" fill={co1} fontSize="8" fontFamily="sans-serif">
                  {di1?.name_acronym}
                </text>
                <text x="5" y="56" fill={co2} fontSize="8" fontFamily="sans-serif">
                  {di2?.name_acronym}
                </text>
              </svg>
            </div>
