...
export default function TourOverlay({ onClose }) {
  ...
  const dismiss = () => {
    onClose();
    try {
      localStorage.setItem("f1s-toured", "1");
    } catch {}
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div style={{ fontSize: 11, color: F1.blue, fontWeight: 900, letterSpacing: "0.15em" }}>
            ΒΗΜΑ {step + 1} ΑΠΟ {TOUR.length}
          </div>
          <button
            onClick={dismiss}
            style={{
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
          {step > 0 && (
            <button onClick={() => setStep((s) => s - 1)} style={{ padding: "6px 16px", fontSize: 11 }}>
              ← ΠΙΣΩ
            </button>
          )}
          {step < TOUR.length - 1 ? (
            ...
            <button onClick={dismiss} className="f1-btn" style={{ padding: "6px 20px", fontSize: 11 }}>
              ΕΝΤΑΞΕΙ 🏁
            </button>
          )}
        </div>
