  ...
  return (
    <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: F1.carbon, border: `1px solid ${F1.blue}33`, borderRadius: 12, padding: 0, zIndex: 100, width: mob ? "95%" : 540, maxHeight: "80vh", display: "flex", flexDirection: "column", animation: "fadeIn .2s", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "16px 20px", borderBottom: `1px solid ${F1.borderLight}` }}>
        <div>
          <div style={{ fontWeight: 900, fontSize: 16, fontFamily: F1.sans, letterSpacing: "0.05em" }}>ΕΠΙΛΕΓΜΕΝΕΣ ΜΑΧΕΣ</div>
          <div style={{ fontSize: 10, color: F1.textMuted, marginTop: 2 }}>Αναζήτηση ανά οδηγό, πίστα, σκέλος ή σεζόν</div>
        </div>
        <button aria-label="Κλείσιμο επιλεγμένων μαχών" onClick={onClose} style={getModalCloseButtonStyle(F1)}>✕</button>
      </div>
      <div style={{ overflowY: "auto", padding: "12px 20px 20px", flex: 1 }}>
        <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1.4fr 1fr", gap: 10, marginBottom: 10 }}>
          ...
          <select value={yearFilter} onChange={(e) => setYearFilter(e.target.value)} style={{ width: "100%", borderRadius: 8, padding: "10px 12px", fontSize: 12 }}>
            ...
            {years.map((yr) => <option key={yr} value={yr}>{yr}{unavailableSet.has(yr) ? " (Μη διαθέσιμη)" : ""}</option>)}
          </select>
        </div>
        ...
        {!!query && (!!matchingDrivers.length || !!matchingCircuits.length) && (
          <div style={{ marginBottom: 12, padding: "10px 12px", borderRadius: 10, background: F1.cardBg, border: `1px solid ${F1.borderLight}` }}>
            <div style={{ fontSize: 10, color: F1.textMuted, marginBottom: 8, letterSpacing: "0.08em", fontWeight: 700 }}>ΓΡΗΓΟΡΑ ΦΙΛΤΡΑ ΑΠΟ ΤΟ ΠΛΗΡΕΣ ΕΥΡΕΤΗΡΙΟ</div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
          <button onClick={() => setYearFilter("all")} style={{ padding: "5px 10px", fontSize: 10, background: yearFilter === "all" ? F1.blue : F1.cardBg, borderColor: yearFilter === "all" ? F1.blue : F1.borderLight, color: yearFilter === "all" ? "#fff" : F1.text }}>ΟΛΑ</button>
          {years.map((yr) => (
          ...
          return (
            <div key={yr} style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "baseline", fontSize: 11, fontWeight: 900, color: F1.blue, letterSpacing: "0.12em", marginBottom: 8, fontFamily: F1.mono, borderBottom: `1px solid ${F1.blue}22`, paddingBottom: 4 }}>
                <span>ΣΕΖΟΝ {yr}</span>
                {yearUnavailable && <span style={{ fontSize: 9, color: F1.textMuted, letterSpacing: "0.04em" }}>Η τηλεμετρία έρχεται σύντομα</span>}
              </div>
              {items.map((p, i) => (
                <button
                  key={i}
                  ...
                  title={yearUnavailable ? "Αυτό το preset θα ενεργοποιηθεί μόλις υπάρξει διαθέσιμη τηλεμετρία για τη σεζόν." : p.label}
                ...
                >
                  ...
                  <div style={{ marginTop: 4, fontSize: 10, color: yearUnavailable ? F1.textMuted : F1.textDim, fontFamily: F1.sans }}>
                    {(DRIVER_NAME_BY_NUMBER[p.d1] || `#${p.d1}`)} εναντίον {(DRIVER_NAME_BY_NUMBER[p.d2] || `#${p.d2}`)} • {p.meeting.replace(" Grand Prix", "")} • {formatSessionLabel(p.session)}
                  </div>
                </button>
              ))}
            </div>
        ...
        {!filteredPresets.length && <div style={{ padding: "18px 6px", fontSize: 12, color: F1.textDim, textAlign: "center", lineHeight: 1.6 }}>Δεν βρέθηκαν presets με αυτά τα φίλτρα. Μπορείς να χρησιμοποιήσεις τα γρήγορα φίλτρα οδηγών και πιστών πιο πάνω για να ψάξεις σε όλο το ευρετήριο.</div>}
      </div>
    </div>
  );
}
