...
export default function PresetsModal({ mob, onClose, onLoadPreset }) {
  const F1 = getF1();
  return (
    <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: F1.carbon, border: `1px solid ${F1.red}33`, borderRadius: 12, padding: 0, zIndex: 100, width: mob ? "95%" : 460, maxHeight: "80vh", display: "flex", flexDirection: "column", animation: "fadeIn .2s", overflow: "hidden" }}>
        ...
        {["2026", "2025", "2024", "2023", "2022", "2021"].map((yr) => {
          ...
          return (
            <div key={yr} style={{ marginBottom: 16 }}>
              ...
              {items.map((p, i) => (
                <button key={i} onClick={() => onLoadPreset(p)} style={{ display: "block", width: "100%", textAlign: "left", padding: "10px 12px", marginBottom: 4, fontSize: 12, fontFamily: F1.mono, borderLeft: `3px solid ${F1.red}`, lineHeight: 1.4 }}>
                  {p.label}
                </button>
              ))}
            </div>
